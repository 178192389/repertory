...
class rediser
{
     ...
     */
    public function __construct($configArr = null){
    	$this->setConfigArr($configArr);
    }
     ...
     * 是否准备好了
     */
    public function isReady($autoConn = false){
    	if($autoConn)
    		$this->getConn();
     ...
     * @param array $configArr 配置数组array($host, $port)
     */
    public function setConfigArr($configArr) {
    	if(count($configArr) >= 2){
	        $this->_configArr = array(
	            'host' => $configArr[0],
	            'port' => $configArr[1],
                'pass' => isset($configArr[2]) ? $configArr[2] : null,
	        );
	    }
	    else{
	    	$this->_configArr = array();
    ...
    public function getConn() {
        if (!isset($this->_r)) {
            ...
            if (!extension_loaded('redis')) {
                App::log()->log_dbg('Extension(redis) not loaded!');
                return null;
                //throw new UserException(UserException::ERROR_SYSTEM, 'Extension(%s) not loaded!', 'redis');
            }
            // 创建Redis的实例
            if(isset($this->_configArr)){
                $this->_r = new Redis();
	            try {
	                if($this->_r->connect($this->_configArr['host'], $this->_configArr['port'], $this->_timeout)){
                        if (!empty($this->_configArr['pass'])) {
                            if(!$this->_r->auth($this->_configArr['pass'])){
                                return false;
                            }
                        }
	                	$this->_isReady = true;
            			//App::log()->log('Redis ready>' . $this->_configArr['host'] . ':' . $this->_configArr['port']);
	                }
	                else{
            			App::log()->log_dbg('Redis failed>' . $this->_configArr['host'] . ':' . $this->_configArr['port']);
                        $this->_r = null;
	                }
	            } catch (Exception $e) {
                    App::log()->log_dbg('Redis failed>' . $e->getMessage());
                        $this->_r = null;
	            }
            }
            else{
                App::log()->log_dbg('Redis failed> no config');
            }
        }
        return $this->_r;
    }
    ...
    public function sRandMember($key,$num=1) { 
        return $this->getConn()->sRandMember($key,$num);
    }
